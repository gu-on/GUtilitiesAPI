...
void LoadPlugin()
{
	...
	Api.Add({APIFUNC(GU_Filesystem_CountMediaFiles), "int", "const char*,int,double*", "path,flags,fileSizeOut", "Recursively counts all media files within a given path. Returns -1 if path is invalid. Can also retrieve fileSize sum in megabytes"});
