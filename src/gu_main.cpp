...
void LoadPlugin()
{
	...
	Api.Add({APIFUNC(GU_PCM_Source_TimeToPeak), "double", "PCM_source*,int,double", "source,bufferSize,threshold", "Returns duration in seconds for PCM_source from start til peak threshold is breached (dBFS). Returns -1 if invalid"});
	Api.Add({APIFUNC(GU_PCM_Source_TimeToRMS), "double", "PCM_source*,int,double", "source,bufferSize,threshold", "Returns duration in seconds for PCM_source from start til RMS threshold is breached (dBFS). Returns -1 if invalid"});
	Api.Add({APIFUNC(GU_PCM_Source_TimeToPeakR), "double", "PCM_source*,int,double", "source,bufferSize,threshold", "Returns duration in seconds for PCM_source from end til peak threshold is breached in reverse (dBFS). Returns -1 if invalid"});
	Api.Add({APIFUNC(GU_PCM_Source_TimeToRMSR), "double", "PCM_source*,int,double", "source,bufferSize,threshold", "Returns duration in seconds for PCM_source from end til RMS threshold is breached in reverse (dBFS). Returns -1 if invalid"});
